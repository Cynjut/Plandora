...
public class ResourceProjectAllocHighchartGadget extends HighchartGadget {
	...
	public String generate(Vector selectedFields)  throws BusinessException {
        ...
        try {
            ...
            if (dbAllocList!=null) {
            	String resourceName = this.getResource(resourceId);
                ...
                labels = this.cropArray(labels);
                this.cropMatrix(valBar, labels.length);
            
        		//draw the chart
                response = super.getHeader(this.getId(), "column", super.getI18nMsg(getUniqueName()), resourceName, false) +
              		   super.getLegend(false) +
              		   super.getXAxis(xaxis, false) +
              		   super.getYAxis(0, "(h)") +
              		   super.getTooltip() +
              		   super.getPlot(labels, valBar, true, null) +
              		   super.getFooter();                  
            }                    
	...
	private String getResource(String resourceId) throws BusinessException {
		String response = "";
		Vector<TransferObject> userlist = super.getAtiveUsers(super.handler.getId());
		for (TransferObject to : userlist) {
			if (to.getId().equals(resourceId)) {
				response = to.getGenericTag();
				break;
			}
		}
		return response;
